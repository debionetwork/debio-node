...
use traits_genetic_analyst_services::{
    GeneticAnalystServiceInfo as GeneticAnalystServiceInfoT, GeneticAnalystServiceOwner, GeneticAnalystServicesProvider,
};
...
impl<AccountId, Hash, Balance> GeneticAnalystService<AccountId, Hash, Balance> {
    pub fn new(
        id: Hash, 
        owner_id: AccountId, 
        info: GeneticAnalystServiceInfo<Balance>
    ) -> Self {
        Self { 
            id, 
            owner_id, 
...
impl<T, AccountId, Hash, Balance> GeneticAnalystServiceInfoT<T, Balance> for GeneticAnalystService<AccountId, Hash, Balance>
where
...
pub mod pallet {
    ...
    use crate::interface::GeneticAnalystServiceInterface;
    use crate::{Currency, GeneticAnalystService, GeneticAnalystServiceInfo, GeneticAnalystServiceOwner};
    ...
    pub type GeneticAnalystServiceOf<T> = GeneticAnalystService<AccountIdOf<T>, HashOf<T>, BalanceOf<T>>;
    ...
    pub type GeneticAnalystServices<T> = StorageMap<_, Blake2_128Concat, HashOf<T>, GeneticAnalystServiceOf<T>>;
    //                                _,  Hasher         ,  Key     ,  Value
    ...
    pub type GeneticAnalystServicesCountByOwner<T> = StorageMap<_, Blake2_128Concat, AccountIdOf<T>, u64>;
    // -----------------------------
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_genetic_analyst_service(
            ...
            match <Self as GeneticAnalystServiceInterface<T>>::create_genetic_analyst_service(&who, &genetic_analyst_service_info) {
                Ok(genetic_analyst_service) => {
                    Self::deposit_event(Event::GeneticAnalystServiceCreated(genetic_analyst_service, who.clone()));
                    Ok(().into())
                }
        ...
        pub fn update_genetic_analyst_service(
            ...
            let who = ensure_signed(origin)?;
            match <Self as GeneticAnalystServiceInterface<T>>::update_genetic_analyst_service(&who, &genetic_analyst_service_id, &genetic_analyst_service_info) {
                Ok(genetic_analyst_service) => {
                    Self::deposit_event(Event::GeneticAnalystServiceUpdated(genetic_analyst_service, who.clone()));
                    Ok(().into())
                }
                Err(error) => Err(error.into()),
            }
        ...
        pub fn delete_genetic_analyst_service(
            ...
            let who = ensure_signed(origin)?;
            match <Self as GeneticAnalystServiceInterface<T>>::delete_genetic_analyst_service(&who, &genetic_analyst_service_id) {
                Ok(genetic_analyst_service) => {
                    Self::deposit_event(Event::GeneticAnalystServiceDeleted(genetic_analyst_service, who.clone()));
                    Ok(().into())
                }
                Err(error) => Err(error.into()),
            }
...
impl<T: Config> GeneticAnalystServiceInterface<T> for Pallet<T> {
    ...
    fn create_genetic_analyst_service(
        ...
        let can_create_genetic_analyst_service = T::GeneticAnalystServiceOwner::can_create_genetic_analyst_service(owner_id);
        if !can_create_genetic_analyst_service {
            return Err(Error::<T>::NotAllowedToCreate);
        }

        let owner_genetic_analyst_service_count = <Self as GeneticAnalystServiceInterface<T>>::genetic_analyst_services_count_by_owner(owner_id);
        let genetic_analyst_service_id = Self::generate_genetic_analyst_service_id(owner_id, owner_genetic_analyst_service_count);

        // Calculate total price
        let mut genetic_analyst_service_info_mut = genetic_analyst_service_info.clone();
        for (idx, price_by_currency) in genetic_analyst_service_info.prices_by_currency.iter().enumerate() {
            genetic_analyst_service_info_mut.prices_by_currency[idx].total_price -= price_by_currency.total_price;
            for price_component in price_by_currency.price_components.iter() {
                genetic_analyst_service_info_mut.prices_by_currency[idx].total_price += price_component.value;
            }

            for additional_price in price_by_currency.additional_prices.iter() {
                genetic_analyst_service_info_mut.prices_by_currency[idx].total_price += additional_price.value;
            }
        }

        let genetic_analyst_service = GeneticAnalystService::new(genetic_analyst_service_id, owner_id.clone(), genetic_analyst_service_info_mut);
    ...
    fn delete_genetic_analyst_service(
        ...
        let genetic_analyst_service = GeneticAnalystServices::<T>::take(genetic_analyst_service_id).unwrap();
...
impl<T: Config> Pallet<T> {
    ...
    pub fn add_genetic_analyst_services_count_by_owner(owner_id: &T::AccountId) {
        let genetic_analyst_services_count = GeneticAnalystServicesCountByOwner::<T>::get(owner_id).unwrap_or(0);
        GeneticAnalystServicesCountByOwner::<T>::insert(owner_id, genetic_analyst_services_count.wrapping_add(1))
    }
    ...
    pub fn sub_genetic_analyst_services_count_by_owner(owner_id: &T::AccountId) {
        let genetic_analyst_services_count = GeneticAnalystServicesCountByOwner::<T>::get(owner_id).unwrap_or(1);
        GeneticAnalystServicesCountByOwner::<T>::insert(owner_id, genetic_analyst_services_count - 1);
    }
}
...
where
    GeneticAnalystServiceOf<T>: traits_genetic_analyst_services::GeneticAnalystServiceInfo<T, Balance>,
{
    type Error = Error<T>;
    type GeneticAnalystService = GeneticAnalystServiceOf<T>;
    ...
    fn delete_genetic_analyst_service(owner_id: &T::AccountId, id: &T::Hash) -> Result<Self::GeneticAnalystService, Self::Error> {
