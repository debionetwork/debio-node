...
pub mod pallet {
	use crate::{interface::GeneticDataInterface, weights::WeightInfo, GeneticData};
	...
	pub type GeneticDataCountByOwner<T> = StorageMap<_, Blake2_128Concat, AccountIdOf<T>, u64>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn add_genetic_data(
			...
			match <Self as GeneticDataInterface<T>>::add_genetic_data(
				...
				Ok(genetic_data) => {
					Self::deposit_event(Event::GeneticDataAdded(genetic_data, who.clone()));
					Ok(().into())
				},
		...
		pub fn update_genetic_data(
			...
			match <Self as GeneticDataInterface<T>>::update_genetic_data(
				...
				Ok(genetic_data) => {
					Self::deposit_event(Event::GeneticDataUpdated(genetic_data, who.clone()));
					Ok(().into())
				},
				Err(error) => Err(error.into()),
			}
		...
		pub fn remove_genetic_data(
			...
			match <Self as GeneticDataInterface<T>>::remove_genetic_data(&who, &genetic_data_id) {
				Ok(genetic_data) => {
					Self::deposit_event(Event::GeneticDataRemoved(genetic_data, who.clone()));
					Ok(().into())
				},
				Err(error) => Err(error.into()),
			}
...
impl<T: Config> GeneticDataInterface<T> for Pallet<T> {
	...
	fn generate_genetic_data_id(
		...
		let mut genetic_data_count_bytes = genetic_data_count.encode();
	...
	fn add_genetic_data(
		...
		Self::add_genetic_data_by_owner(owner_id, &genetic_data_id);
	...
	fn remove_genetic_data(
		...
		Self::sub_genetic_data_by_owner(genetic_data.get_owner_id(), genetic_data_id);
	...
	fn genetic_data_by_owner_id(owner_id: &T::AccountId) -> Option<Vec<T::Hash>> {
	...
	fn genetic_data_by_id(genetic_data_id: &Self::GeneticDataId) -> Option<Self::GeneticData> {
...
impl<T: Config> Pallet<T> {
	...
	pub fn add_genetic_data_by_owner(owner_id: &T::AccountId, genetic_data_id: &T::Hash) {
		let mut genetic_data = GeneticDataByOwner::<T>::get(owner_id).unwrap_or_default();

		genetic_data.push(*genetic_data_id);
		GeneticDataByOwner::<T>::insert(owner_id, &genetic_data)
	}
	...
	pub fn add_genetic_data_count() {
		let genetic_data_count = <GeneticDataCount<T>>::get().unwrap_or(0);
		<GeneticDataCount<T>>::put(genetic_data_count.wrapping_add(1));
	}

	// Add genetic_data count by owner
	pub fn add_genetic_data_count_by_owner(owner_id: &T::AccountId) {
		let genetic_data_count = GeneticDataCountByOwner::<T>::get(owner_id).unwrap_or(0);
		GeneticDataCountByOwner::<T>::insert(owner_id, genetic_data_count.wrapping_add(1))
	}

	// Subtract genetic_data count
	pub fn sub_genetic_data_count() {
		let genetic_data_count = <GeneticDataCount<T>>::get().unwrap_or(1);
