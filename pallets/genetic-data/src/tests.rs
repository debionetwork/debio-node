...
fn remove_genetic_data_works() {
	ExternalityBuilder::build().execute_with(|| {
		...
		assert_ok!(GeneticData::remove_genetic_data(RuntimeOrigin::signed(1), genetic_data_ids[0]));
...
fn remove_genetic_data_not_genetic_data_owner() {
	ExternalityBuilder::build().execute_with(|| {
		...
		assert_noop!(
			GeneticData::remove_genetic_data(RuntimeOrigin::signed(2), genetic_data_ids[0]),
