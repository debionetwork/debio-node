...
fn add_menstrual_data_works() {
	ExternalityBuilder::build().execute_with(|| {
		assert_ok!(MenstrualData::add_menstrual_data(
			Origin::signed(1),
		...
		let menstrual_data_ids = MenstrualData::menstrual_data_by_owner_id(1).unwrap();

		assert_eq!(MenstrualData::menstrual_data_count(), Some(1));

		assert_eq!(MenstrualData::menstrual_data_count_by_owner(1), Some(1));

		let menstrual_data = MenstrualData::menstrual_data_by_id(menstrual_data_ids[0]).unwrap();
...
fn remove_menstrual_data_works() {
	ExternalityBuilder::build().execute_with(|| {
		...
		assert_ok!(MenstrualData::remove_menstrual_data(Origin::signed(1), menstrual_data_ids[0]));

		assert_eq!(MenstrualData::menstrual_data_count(), Some(0));

		assert_eq!(MenstrualData::menstrual_data_count_by_owner(1), Some(0));
	})
}
...
fn remove_menstrual_data_does_not_exist() {
	ExternalityBuilder::build().execute_with(|| {
		assert_noop!(
			MenstrualData::remove_menstrual_data(
				Origin::signed(1),
				Keccak256::hash("0xDb9Af2d1f3ADD2726A132AA7A65Cc9E6fC5761C3".as_bytes())
			),
			Error::<Test>::MenstrualDataDoesNotExist
		);
	})
}
...
fn remove_menstrual_data_not_menstrual_data_owner() {
	ExternalityBuilder::build().execute_with(|| {
		...
		assert_noop!(
			MenstrualData::remove_menstrual_data(Origin::signed(2), menstrual_data_ids[0]),
			Error::<Test>::NotMenstrualDataOwner
		);
	})
}
...
fn update_menstrual_data_works() {
	ExternalityBuilder::build().execute_with(|| {
		...
		assert_ok!(MenstrualData::update_menstrual_data(
			Origin::signed(1),
...
fn update_menstrual_data_does_not_exist() {
	ExternalityBuilder::build().execute_with(|| {
		assert_noop!(
			MenstrualData::update_menstrual_data(
				Origin::signed(1),
				Keccak256::hash("0xDb9Af2d1f3ADD2726A132AA7A65Cc9E6fC5761C3".as_bytes()),
				"DeBio Menstrual Data".as_bytes().to_vec(),
				"DeBio Menstrual Data Document Description".as_bytes().to_vec(),
