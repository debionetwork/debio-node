...
pub fn create_extrinsic(
	...
	function: impl Into<debio_runtime::Call>,
	nonce: Option<u32>,
) -> debio_runtime::UncheckedExtrinsic {
	...
	let period = debio_runtime::BlockHashCount::get()
		.checked_next_power_of_two()
		.map(|c| c / 2)
		.unwrap_or(2) as u64;
	let tip = 0;
	let extra: debio_runtime::SignedExtra = (
		frame_system::CheckSpecVersion::<debio_runtime::Runtime>::new(),
		frame_system::CheckTxVersion::<debio_runtime::Runtime>::new(),
		frame_system::CheckGenesis::<debio_runtime::Runtime>::new(),
		frame_system::CheckEra::<debio_runtime::Runtime>::from(generic::Era::mortal(
			period,
			best_block.saturated_into(),
		)),
		frame_system::CheckNonce::<debio_runtime::Runtime>::from(nonce),
		frame_system::CheckWeight::<debio_runtime::Runtime>::new(),
		pallet_transaction_payment::ChargeTransactionPayment::<debio_runtime::Runtime>::from(tip),
	);

	let raw_payload = debio_runtime::SignedPayload::from_raw(
		function.clone(),
		extra.clone(),
		(
			debio_runtime::VERSION.spec_version,
			debio_runtime::VERSION.transaction_version,
			genesis_hash,
			best_hash,
			(),
			(),
	...
	debio_runtime::UncheckedExtrinsic::new_signed(
		function.clone(),
		sp_runtime::AccountId32::from(sender.public()).into(),
		debio_runtime::Signature::Sr25519(signature.clone()),
		extra.clone(),
	)
}
